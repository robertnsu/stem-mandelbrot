    ...
    static *tiles(width, height, numRows, numColumns) {
        ...
        for (let row = 0; row < numRows; row++) {
            ...
            for (let column = 0; column < numColumns; column++) {
                let tileWidth = (column < numColumns - 1) ? columnWidth : width - columnWidth * (numColumns - 1);

                yield new Tile(column * columnWidth, row * rowHeight, tileWidth, tileHeight);
            }
        }
    }
}
...
class MandelbrotCanvas {
    ...
    setSize() {
        ...
        this.tiles = [...Tile.tiles(this.width, this.height, ROWS, COLUMNS)];
    }
    ...
    setState(f, save=true) {
        if (typeof f === "function") {
            ...
            for (let property in f) {
                this.state[property] = f[property];S
            }
        }
    ...
    handleKey(event) {
        switch (event.key) {
            case "Escape":  // go back to initial state
                this.setState(PageState.initialState());
                break;
            case "+":       // increase max iterations
                this.setState(s => { s.maxIterations = Math.round(s.maxIterations * 1.5); });
                break;
            case "-":       // decrease max iterations
                this.setState(s => {
